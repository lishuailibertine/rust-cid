...
/// Vec<Cid> IPLD DagJSON serialization/deserialization.
pub mod vec {
    use super::*;

    #[derive(Serialize)]
    struct JsonCidRef<'a>(#[serde(with = "super")] &'a Cid);

    /// IPLD DagJSON serialization of Vec<Cid>.
    ...
    #[derive(Deserialize)]
    struct JsonCid(#[serde(with = "super")] Cid);

    /// IPLD DagJSON deserialization of Vec<Cid>.
