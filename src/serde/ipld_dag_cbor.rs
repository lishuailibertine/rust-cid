    ...
    S: ser::Serializer,
{
    ...
    Tagged::new(Some(IPLD_DAG_CBOR_TAG_CID), value).serialize(serializer)
}
    ...
    D: de::Deserializer<'de>,
{
    ...
    match tagged.tag {
        Some(IPLD_DAG_CBOR_TAG_CID) | None => {
            ...
            Ok(Cid::try_from(&bytes[1..]).map_err(|e| de::Error::custom(e.to_string()))?)
        }
        Some(_) => Err(de::Error::custom("unexpected CBOR tag")),
    }
}

/// Vec<Cid> IPLD DagCBOR serialization/deserialization.
pub mod vec {
    use super::*;

    #[derive(Serialize)]
    struct CborCidRef<'a>(#[serde(with = "super")] &'a Cid);

    /// IPLD DagCBOR serialization of Vec<Cid>.
    ...
    #[derive(Deserialize)]
    struct CborCid(#[serde(with = "super")] Cid);

    /// IPLD DagCBOR deserialization of Vec<Cid>.
